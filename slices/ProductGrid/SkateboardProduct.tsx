...
const VETICAL_LINE_CLASSES = "absolute top-0 h-full stroke-2 text-stone-300 transition-colors group-hover:text-stone-400";
...
export async function SkateboardProduct({id}: Props) {
    ...
    const price = isFilled.number(product.data.price) ? `${(product.data.price / 100).toFixed(2)}`
    ...
    return (
        <div className='group relative mx-auto w-full max-w-72 px-8 pt-4'>
            <VerticalLine className={clsx(VETICAL_LINE_CLASSES, "left-4")} />
            <VerticalLine className={clsx(VETICAL_LINE_CLASSES, "right-4")} />
            ...
            <div className='-mb-1 overflow-hidden py-4'>
                ...
                alt='' 
                field={product.data.image} width={150} />
            </div>
            <HorizontalLine className={HORIZONTAL_LINE_CLASSES}/>
            ...
            className="absolute inset-0 flex items-center justify-center
                ...
                <ButtonLink field={product.data.customizer}>
                    Customize
                </ButtonLink>
            </div>
        </div>
